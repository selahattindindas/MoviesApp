...
import { PlatformEnum } from 'src/app/enums/Platform';
...
export class UpdateComponent  implements OnInit{
  ...
  movie: Update_Movie;
  constructor(private fb: FormBuilder,private categoryService: CategoryService,private platformService:PlatformService,
    private movieService: MoviesService,private activatedRoute: ActivatedRoute) 
  {
...
getMovieById() {
  this.activatedRoute.params.subscribe(async (params) => {
      const movieData: Partial<Update_Movie> = await this.movieService.getMovieId(params['id']);
      if (movieData && movieData.name)
        this.movie = movieData as Update_Movie;
  });
}
updateId(){
