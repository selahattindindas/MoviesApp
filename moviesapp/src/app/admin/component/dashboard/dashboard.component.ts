...
export class DashboardComponent implements AfterViewInit {
  ...
  dateCharts() {
    ...
    this.movieservice.getAllMovies().then((movies: List_Movie[]) => {
      ...
      const categorizeMovies = (movieDate: Date, dateEnum: DateEnum): boolean => {
        switch (dateEnum) {
          case DateEnum.Past:
      ...
      const visionMovies = movies.filter(movie => categorizeMovies(parseDate(movie.releaseDate), DateEnum.Vision));
      const noMovies = movies.filter(movie => categorizeMovies(parseDate(movie.releaseDate), DateEnum.Past));
      const commingMovies = movies.filter(movie => categorizeMovies(parseDate(movie.releaseDate), DateEnum.Soon));
